...
class Droplet:
    ...
    def _convert_raw_torrent(self, torrent):
        ...
        if b'length' in torrent[0][b'info']:
            ...
            self.length = None
        self.name = torrent[0][b'info'][b'name'].decode(ENCODING)
        self.piece_length = torrent[0][b'info'][b'piece length']
        self.files = []
        if b'files' in torrent[0][b'info']:
            for x in torrent[0][b'info'][b'files']:
                self.files.append((x[b'path'][0].decode(ENCODING), x[b'length'])) #TODO: Is filepath always first and only item in list?
    ...
    def add_associated_file(self, file_tuple):
        # root = file_tuple[ROOT_INDEX]
        # filename = file_tuple[FILENAME_INDEX]
